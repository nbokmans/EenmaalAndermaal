...
$number = array_key_exists('number', $_GET) ? $_GET['number'] : null;
...
<div id="content">
	...
	<nav class="subNavigatie">
		<?php
		$subrubrieken = Rubriek::getSubRubriekName($number);
		foreach ($subrubrieken as $sub)
		{
			?>
			<a href="productensubrubriek.php?rubrieknumber=<?php echo $number ?>&subNumber=<?php echo $sub['rubrieknummer'] ?>"><?php echo $sub['Rubrieknaam'] ?></a>
		<?php
		}
		?>
	</nav>
	<?php
	$voorwerp = Voorwerp::getVoorwerpInRubriek($number, 500, $searchProduct);

	if ($voorwerp === null && isset($_GET['number']))
	{
			...
			foreach ($voorwerpImage as $imagenumber)
			{
				...
				<div class="product">
					...
					<div class="productContent">
						...
						<p><?php
							$gesloten = Voorwerp::getAuctionStatus((float)$element['voorwerpnummer']);
							echo $gesloten;
							?>
